        ...
        if p[0] == '$':
            if p == '$prefix':
                ...
                if passed_prefix:
                    name_loc = [
                        n
                        for (n, e) in enumerate(name[index:])
...
def set_slices_and_vars_time_series(
    directory,
    file_pattern,
    date_pattern,
    prefix,
...
def set_slices_and_vars_depend(
    directory, file_pattern, prefix, start_yr, end_yr, ave_type, ave, region
):
